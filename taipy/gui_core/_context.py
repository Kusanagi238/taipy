...
class _GuiCoreContext(_CoreEventConsumerBase):
    ...
    def broadcast_core_changed(self, payload: t.Dict[str, t.Any], client_id: t.Optional[str] = None):
        try:
            # Guard broadcast in case the GUI server isn't started or internals are missing
            if hasattr(self.gui, "_broadcast"):
                self.gui._broadcast(_GuiCoreContext._CORE_CHANGED_NAME, payload, client_id)  # type: ignore
        except AttributeError:
            # If the Gui instance doesn't have the underlying server attribute, ignore broadcasting
            # to avoid raising during shutdown or before server start.
            pass
    ...
    def edit_entity(self, state: State, id: str, payload: t.Dict[str, str]):
        ...
        if scenario:
            try:
                # If a specific sequence name is provided, operate on that sequence (edit/rename/delete)
                if isinstance(sequence, str):
                    ...
                    # No specific sequence provided: either add a new sequence (when name present)
                    # or edit scenario-level properties (including setting primary)
                    if (name := data.get(_GuiCoreContext.__PROP_ENTITY_NAME)) and isinstance(name, str):
    ...
    def _auth_listener(self, state: State, client_id: t.Optional[str], payload: t.Dict[str, t.Any]):
        # Prepare message based on authorization check
        msg = (
        ...
        try:
            ...
            # If the Gui server isn't available, ignore broadcasting to avoid errors during startup/shutdown
            pass
